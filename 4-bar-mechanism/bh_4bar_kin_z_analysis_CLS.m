classdef bh_4bar_kin_z_analysis_CLS
%_#########################################################################
% EXPECTED USAGE:
%
%  Initialization:
%    >> OBJ_kin = bh_4bar_kin_z_analysis_CLS(L1, L2, L3, L4)
%    >> OBJ_kin = bh_4bar_kin_z_analysis_CLS(L1, L2, L3, L4, theta_rad)
%
...
%    >> OBJ_kin.plot_z()
    ...
    %----------------------------------------------------------------------
    methods
        function obj = bh_4bar_kin_z_analysis_CLS(L1,L2,L3,L4,theta_rad)
          % ALLOWED USAGE:
          %   >> obj = bh_4bar_kin_CLS()
        ...
        function  tf_is_valid_pose_col = is_valid_pose(obj, theta_rad_vec)
           ...
           tf_is_valid_pose_col = false(N,1);
           ...
           for kk=1:length(theta_rad_vec)
              ...
              [the_z, the_num, the_den] = bh_z_calc_util(L_1, L_2, L_3, L_4, theta_rad, "CALC_Z");               
               tf_is_real     = isreal(the_z);
               A_SMALL_NUMBER = 1e-16;
                              
               if(tf_is_real & all(isfinite(the_z)) )
                   tf_is_valid_pose_col(kk) = true;
               else
                    tf_is_valid_pose_col(kk) = false;
               end
        ...
        function [phi_pair_rad, alpha_pair_rad] = calc_phi_alpha_rad(obj, theta_rad)
           ...
           tf_is_valid_pose = is_valid_pose(obj, theta_rad);
           
           if(true==tf_is_valid_pose)           
              [phi_pair_rad, alpha_pair_rad] = ...
                  bh_angle_calc_util(L_1, L_2, L_3, L_4, theta_rad, "CALC_PHI_AND_ALPHA_VEC"); 
           else
              phi_pair_rad   = NaN(2,1);
              alpha_pair_rad = NaN(2,1);
           end
        end
        %------------------------------------------------------------------
        ...
        function [phi_rad_value, alpha_rad_value] = get_phi_alpha(obj, theta_rad, act_str)
           ...
           if(2==nargin)
                [phi_pair_rad, alpha_pair_rad] = bh_angle_calc_util(L_1, L_2, L_3, L_4, theta_rad, "CALC_PHI_AND_ALPHA_VEC");          
               ...
               [phi_rad_value, alpha_rad_value] = ...
                   bh_angle_calc_util(L_1, L_2, L_3, L_4, theta_rad, act_str);
           end
        end
        %------------------------------------------------------------------
        function plot_valid_theta(obj)
           if(1==nargin)
               figure;
               hax(1) = subplot(1,2,1);
               hax(2) = subplot(1,2,2);
           ...
           scatter(hax(1),x, y, [], c_rgb);
           xlim(hax(1),[-1,1]);
           ylim(hax(1),[-1,1]);
           
           axis(hax(1),'equal');
           grid(hax(1),'on');
           title(hax(1), 'Valid angular values for \theta');
           ...
           scatter(hax(2),th_deg, tf_vec, [], c_rgb);
           xlim(hax(2), [min(obj.tgt_theta_deg_col), max(obj.tgt_theta_deg_col)]);
           ylim(hax(2), [0, 1.1])
           grid(hax(2),'on');
           title(hax(2), 'Valid angular values for \theta');
           xlabel(hax(2), '\theta (deg)')
        end
        %------------------------------------------------------------------                        
...
%                   hold(hax_alpha, 'on')
%                   
%                   hL_theta = plot(hax_theta, [0,1],[0,0],'-r', 'LineWidth',3,'Visible','off', 'Tag', 'TAG_LINE_THETA');
%                   hL_phi   = plot(hax_phi,   [0,1],[0,0],'-b', 'LineWidth',3,'Visible','off', 'Tag', 'TAG_LINE_PHI');
%                   hL_alpha = plot(hax_alpha, [0,1],[0,0],'-g', 'LineWidth',3,'Visible','off', 'Tag', 'TAG_LINE_ALPHA');
%                end
%                
%                tgt_theta_deg = obj.tgt_theta_deg_col(kk);
%                
...
%                y         = sind(tgt_theta_deg);
% 
%                set(hL_theta, 'XData',[0,x],'YData',[0,y],'Visible','on');
%                
%                if(~obj.tf_tgt_is_valid_pose_col(kk))
%                    hL_phi.Visible   = 'off';
%                    hL_alpha.Visible = 'off';
%                    set(hax_AN,'Color',[1,0.8,0.5]);
%                    drawnow
%                    pause(pause_time_for_invalid/1000);
%                    continue
...
%                    
%                    %--------------------------------------------------------------------
%                    %LOC_plot_for_animate(   hax_AN,A,B1,B2,C,D);  % draw the 4bar mechanism
%                    LOC_plot_for_animate_v2(hax_AN,A,B1,B2,C,D);  % draw the 4bar mechanism
%                    %--------------------------------------------------------------------
%                    x = cosd(phi_deg);  y = sind(phi_deg);
%                    set(hL_phi, 'XData',[0,x],'YData',[0,y],'Visible','on');
%                   
%                    x = cosd(alpha_deg);  y = sind(alpha_deg);
%                    set(hL_alpha, 'XData',[0,x],'YData',[0,y],'Visible','on');
%                     
% %                    b = toc(a);
% %                    if b > (1/10)
